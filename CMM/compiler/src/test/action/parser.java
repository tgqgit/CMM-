...
public class parser {
    ...
    //田国庆加的
    public boolean last = true;//TGQ
    public static int elseError = 1;   //返回值不是int的函数中错误
    //读取下一个token
    public void nextSyn(){
        ...
        if(syn.kind == 6 || syn.kind == 7){   //数据类型
            ...
            if(syn.kind == 24){    //变量名
                ...
                if(syn.kind == 19){  //  ;
                    ...
                    result = 1;
                    elseError = 0;  //TGQ
                    System.out.println(syn.line + "行声明语句缺少“;”");
        ...
        if(syn.kind == 22){  //数组声明 [int值]  [
            ...
            if(syn.kind == 25){  //int值
                ...
                if(syn.kind == 23){  // ]
                    ...
                    entrance.parserResult = entrance.parserResult + "数组声明错误！-缺少“]”\n";
                    result = 1;
                }
            }
            else{
    ...
    public int assStatement(){
        ...
        if(syn.kind == 24){   //变量名
            ...
            if(syn.kind == 19 && result == 1){
                ...
                entrance.parserResult = entrance.parserResult + "赋值语句分析正确\n";
            }
            else{
                elseError = 0;  //TGQ
